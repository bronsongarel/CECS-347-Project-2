...
void ADC0_InitSWTriggerSeq3_Ch1(void){ 
  SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R4;   // 1) activate clock for Port E
	...
	GPIO_PORTE_DIR_R &= ~RIGHT_IR; 										// 2) make PE2 input
  GPIO_PORTE_AFSEL_R |= RIGHT_IR;										// 3) enable alternate function on PE2
  GPIO_PORTE_DEN_R &= ~RIGHT_IR; 										// 4) disable digital I/O on PE2
  GPIO_PORTE_AMSEL_R |= RIGHT_IR;										// 5) enable analog function on PE2
		
  SYSCTL_RCGC0_R |= 0x00010000;   									// 6) activate ADC0 
	while ((SYSCTL_RCGC0_R&0x00010000)!=0x00010000){}
         
  SYSCTL_RCGC0_R &= ~SYSCTL_RCGC0_ADC0SPD_1M;  			// 7) configure for 125K 
  ADC0_SSPRI_R = 0x0123;          									// 8) Sequencer 3 is highest priority
  ...
  ADC0_SSCTL3_R = ADC_SSCTL3_END0
		| ADC_SSCTL3_IE0;         											// 12) no TS0 D0, yes IE0 END0
  ADC0_ACTSS_R |= ADC_RIS_INR3;         						// 13) enable sample sequencer 3
}
